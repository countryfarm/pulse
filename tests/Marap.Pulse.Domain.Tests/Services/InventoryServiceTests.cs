...
public class InventoryServiceTests
{
  ...
  public void ReceiveStock_ShouldAddStockItem_AndMarkLineReceived()
  {
    ...
    var line = PurchaseOrderLineFactory.CreateWithId(PurchaseOrderLineId.From(1), part.Id, Quantity.From(10m), po.Id);
    ...
    line.ReceivedQuantity.Should().Be(Quantity.From(10m));
  ...
  {
    var part = PartFactory.CreateWithId(PartId.From(2), "SKU-002", "MPN-002", "Test Part", Quantity.From(5m));
    var location = LocationFactory.CreateWithId(LocationId.From(1), "Main Bin", "Bin");
    var service = new InventoryService();

    // Add two stock items with different ReceivedAt
    var older = StockItemFactory.CreateWithId(StockItemId.From(1), part.Id, location.Id, Quantity.From(5m), DateTime.UtcNow.AddDays(-2));
    var newer = StockItemFactory.CreateWithId(StockItemId.From(2), part.Id, location.Id, Quantity.From(10m), DateTime.UtcNow);
    part.AddStock(older);
    part.AddStock(newer);

    service.ConsumeStock(part, Quantity.From(8m));
  ...
  {
    var part = PartFactory.CreateWithId(PartId.From(3), "SKU-003", "MPN-003", "Test Part", Quantity.From(5m));
    var location = LocationFactory.CreateWithId(LocationId.From(1), "Main Bin", "Bin");
    var service = new InventoryService();

    var stockItem = StockItemFactory.CreateWithId(StockItemId.From(1), part.Id, location.Id, Quantity.From(3m), DateTime.UtcNow);
    part.AddStock(stockItem);

    Action act = () => service.ConsumeStock(part, Quantity.From(10m));
